...
static void freeze_screen(void) {
    SDL_Event event;
    for (;;) {
 ...
 */
static void get_rand_pos(SDL_FRect *rect, struct snake *snake,
                         uint32_t cell_size, size_t rows, size_t columns) {
    bool overlapping;
    do {
        overlapping = false;
...
int main(int argc, char **argv) {
    ...
    uint32_t window_width = columns * cell_size;
    uint32_t window_height = rows * cell_size;
