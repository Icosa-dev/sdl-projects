...
void get_user_input(struct snake *snake, const struct keybinds *keybinds,
                    bool *running) {
    SDL_Event event;
 ...
 * possible path
 */
static enum direction get_longest_direction(const struct snake *snake,
                                            uint32_t cell_size,
 ...
 */
void get_cpu_input(struct snake *snake, bool *running, uint32_t cell_size,
    ...
    snake->direction =
        get_longest_direction(snake, cell_size, window_width, window_height);
}
