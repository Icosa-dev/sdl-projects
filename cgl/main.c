 ...
 */
static bool **alloc_grid(size_t rows, size_t columns) {
 ...
 */
static void free_grid(bool **grid, size_t rows) {
    for (size_t i = 0; i < rows; i++)
        free(grid[i]);
    free(grid);
}
 ...
 */
static uint32_t count_neighbors(bool **grid, size_t rows, size_t columns,
                                size_t r, size_t c) {
    uint32_t count = 0;
 ...
 * @param columns The amount of columns in the grid
 */
static void update_grid(bool **grid, size_t rows, size_t columns) {
    bool **tmp = alloc_grid(rows, columns);

    for (size_t r = 0; r < rows; r++) {
        for (size_t c = 0; c < columns; c++) {
            uint32_t count = count_neighbors(grid, rows, columns, r, c);
 ...
 * @param columns The amount of columns in the grid
 */
static void generate_random_grid(bool **grid, size_t rows, size_t columns) {
    srand(time(NULL));
